...
def train(args):
    ...
    if args.colocate_ref_reward:
        assert (
            args.ref_num_nodes == args.reward_num_nodes
            and args.ref_num_gpus_per_node == args.reward_num_gpus_per_node
        ), f"num_nodes and num_gpus_per_node must be the same when colocate reference and reward model."

        bundles = [
            {"GPU": args.ref_num_gpus_per_node, "CPU": args.ref_num_gpus_per_node}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--actor_num_gpus_per_node", type=int, default=1, help="number of gpus per node for actor"
    )
    parser.add_argument("--critic_num_nodes", type=int, default=1, help="number of nodes for critic")
    parser.add_argument(
        "--critic_num_gpus_per_node", type=int, default=1, help="number of gpus per node for critic"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--prompt_data", type=str, default="/lustre/fw/datasets/antropic-hh/ppo_prompt_only.jsonl"
    )
    parser.add_argument(
        "--prompt_data_probs",
        type=str,
